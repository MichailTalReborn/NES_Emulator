...
olc::Sprite &olc2C02::GetNameTable(uint8_t i) { return *sprNameTable[i]; }
...
uint8_t olc2C02::ppuRead(uint16_t addr, bool rdonly) {
  ...
  if (cart->ppuRead(addr, data)) {
    ...
    data = tblPattern[(addr & 0x01000) >> 12][addr & 0x0FFF];
...
void olc2C02::clock() {
  ...
  if (scanline >= -1 && scanline < 240) {
    ...
    if ((cycle >= 2 && cycle < 258) || (cycle >= 321 && cycle < 338)) {
      ...
      case 4:
        bg_next_tile_lsb =
            ppuRead((control.pattern_background << 12) +
                    ((uint16_t)bg_next_tile_id << 4) + (vram_addr.fine_y) + 0);
      case 6:
        bg_next_tile_msb =
            ppuRead((control.pattern_background << 12) +
    ...
    if (cycle == 340) {
      for (uint8_t i = 0; i < sprite_count; i++) {
        ...
        if (!control.sprite_size) {
          // 8x8 Sprite Mode - controlled by the register
  ...
  if (mask.render_background) {
    ...
    uint8_t p0_pixel = (bg_shifter_attrib_lo & bit_mux) > 0;
    uint8_t p1_pixel = (bg_shifter_attrib_hi & bit_mux) > 0;
