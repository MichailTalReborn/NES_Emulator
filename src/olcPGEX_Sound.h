...
public:
  ...
  public:
    ...
    olc::rcode LoadFromFile(std::string sWavFile,
...
public:
  static bool InitialiseAudio(unsigned int nSampleRate = 44100,
                              unsigned int nChannels = 1,
                              unsigned int nBlocks = 8,
                              unsigned int nBlockSamples = 512);
  static bool DestroyAudio();
  static void
  SetUserSynthFunction(std::function<float(int, float, float)> func);
  static void
  SetUserFilterFunction(std::function<float(int, float, float)> func);

public:
  static int LoadAudioSample(std::string sWavFile,
                             olc::ResourcePack *pack = nullptr);
...
private:
  ...
  static void CALLBACK waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1,
                                   DWORD dwParam2);
...
olc::rcode SOUND::AudioSample::LoadFromFile(std::string sWavFile,
  ...
  auto ReadWave = [&](std::istream &is) {
    ...
    is.read(
        (char *)&wavHeader,
        nHeaderSize); // sizeof(WAVEFORMATEX)); // Read Wave Format Structure
                      // chunk Note the -2, because the structure has 2 bytes to
                      // indicate its own size which are not in the wav file
    ...
    nSamples =
        nChunksize / (wavHeader.nChannels * (wavHeader.wBitsPerSample >> 3));
...
void SOUND::SetUserFilterFunction(
...
void SOUND::StopSample(int id) {
  ...
  auto s = std::find_if(listActiveSamples.begin(), listActiveSamples.end(),
                        [&](const olc::SOUND::sCurrentlyPlayingSample &s) {
  ...
  for (auto &s : listActiveSamples) {
    if (m_bAudioThreadActive) {
      if (s.bFlagForStop) {
        ...
        s.nSamplePosition +=
            long(roundf((float)vecAudioSamples[s.nAudioSampleID - 1]
        ...
        if (s.nSamplePosition < vecAudioSamples[s.nAudioSampleID - 1].nSamples)
          fMixerSample +=
              vecAudioSamples[s.nAudioSampleID - 1]
                  .fSample[(s.nSamplePosition *
                            vecAudioSamples[s.nAudioSampleID - 1].nChannels) +
                           nChannel];
        else {
          if (s.bLoop) {
            s.nSamplePosition = 0;
...
namespace olc {
bool SOUND::InitialiseAudio(unsigned int nSampleRate, unsigned int nChannels,
  ...
  if (waveOutOpen(&m_hwDevice, WAVE_MAPPER, &waveFormat,
                  (DWORD_PTR)SOUND::waveOutProc, (DWORD_PTR)0,
...
void CALLBACK SOUND::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1,
                                 DWORD dwParam2) {
  if (uMsg != WOM_DONE)
    return;
  m_nBlockFree++;
...
// 'blocks' with audio data. If no requests are available it goes dormant until
// the sound card is ready for more data. The block is fille by the "user" in
// some manner and then issued to the soundcard.
void SOUND::AudioThread() {
  ...
  while (m_bAudioThreadActive) {
    ...
    waveOutPrepareHeader(m_hwDevice, &m_pWaveHeaders[m_nBlockCurrent],
                         sizeof(WAVEHDR));
...
void SOUND::AudioThread() {
  ...
  while (m_bAudioThreadActive) {
    ...
    for (unsigned int n = 0; n < m_nBlockSamples; n += m_nChannels) {
      ...
      for (unsigned int c = 0; c < m_nChannels; c++) {
        nNewSample =
            (short)(clip(GetMixerOutput(c, m_fGlobalTime, fTimeStep), 1.0) *
                    fMaxSample);
        m_pBlockMemory[n + c] = nNewSample;
        nPreviousSample = nNewSample;
      }
    ...
    alBufferData(m_qAvailableBuffers.front(),
                 m_nChannels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16,
                 m_pBlockMemory, 2 * m_nBlockSamples, m_nSampleRate);
...
// Stop and clean up audio system
bool SOUND::DestroyAudio() { return false; }
